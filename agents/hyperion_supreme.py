...
# ══════════════════════════════════════════════════════════════════════════════
GATEWAY_VM          = 1.24   
GATEWAY_DIST_THRESH = 28.0   

PRESSURE_FRONT_MIN  = 2      
PRESSURE_BONUS_PER  = 0.16   
PRESSURE_MAX_MULT   = 1.58   
...
CONCENTRATION_MIN_PROD = 3   

PROACT_HORIZON     = 16
PROACT_RATIO       = 0.28    
MULTI_PROACT_HOR   = 20
MULTI_PROACT_RATIO = 0.36    
MULTI_STACK_WIN    = 5
REACT_MY_TOP_K     = 4
REACT_EN_TOP_K     = 4
PROACT_EN_TOP_K    = 3
...
ONE_V_ONE_AGG_RESERVE = 0.28   

RUSH_DETECT_STEP_MAX = 70
RUSH_FLEET_MIN       = 26    
RUSH_HOME_ETA_MAX    = 28

INTERCEPT_ETA_MAX   = 30    
INTERCEPT_ENABLED   = True

WIN_SECURE_RATIO     = 1.28   
WIN_DESPERATE_RATIO  = 0.76
WIN_SECURE_MARGIN_M  = 0.80   
WIN_DESPERATE_RISK_M = 1.32   
...
def aim_with_prediction(src, target, ships, init_by_id, ang_vel, comets, comet_ids):
    ...
    for _ in range(7):  # HYPERION: 7-iteration convergence
        _, turns = e
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, init_by_id, ang_vel, comets, comet_ids):
        ...
        self.in_blitz     = (step < OPENING_BLITZ_TURNS)   # HYPERION BLITZ
...
def preferred_send(target, needed, arr_t, cap, world, modes, policy, distance=None):
    ...
    if world.in_blitz: m = max(0, int(m * 0.65))   
    base = min(cap, send + m)
...
def plan_moves(world, deadline=None):
    ...
    if (CONCENTRATION_ENABLED and world.eco_mode in EcoMode.SNOWBALL
